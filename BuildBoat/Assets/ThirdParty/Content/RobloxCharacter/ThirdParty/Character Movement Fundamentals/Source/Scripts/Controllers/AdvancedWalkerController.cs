...
using Goossyaa;
using Goossyaa.Third_party.Content.Character_Movement_Fundamentals.Source.Scripts.Input.Character;
...
namespace CMF
{
    ...
    public class AdvancedWalkerController : Controller, ICharacterMovement
    {
     ...
     //   public Transform _point;
        ...
        public Transform CameraTransform
        {
            get
            {
                ...
                {
                    _cameraTransform = FindObjectOfType<FollowPlayerCamera>().Camera;
                }
        ...
        public event Action PressJumpKey;
        ...
        private IEnumerator FallMove()
        {
            //float y = mover.Velocity.y;
...
//            Debug.Log(y + " : height!");
            ...
            if (currentControllerState == ControllerState.Falling || currentControllerState == ControllerState.Jumping || currentControllerState == ControllerState.Rising)
            {
                if (_isCoolDown == false)
                {
                    _coroutine = StartCoroutine(FallMove());
                }
            }

            if (currentControllerState == ControllerState.Grounded && _falling == false)
            {
               // Debug.Log("GROUNDED!!!");
                _isCoolDown = false;
                canJumpUp = false;
            ...
            // else
            // {
            //     mover.SetVelocity(new Vector3(_velocity.x, direction.y, _velocity.z));
            // }
        }
        ...
        protected virtual Vector3 CalculateMovementDirection()
        {
            ...
            //if(characterInput == null)
            //	return Vector3.zero;
            if (TouchControls_ManipulateCharacter.Instance == null)
                return Vector3.zero;

            Vector3 _velocity = Vector3.zero;

            Vector2 direction = _velocityCharacter.GenerateCharacterVelocity() * (CanPressMovement ? 1 : 0);

            //If no camera transform has been assigned, use the character's transform axes to calculate the movement direction;
            if (CameraTransform == null)
            {
                if (TouchControls_ManipulateCharacter.Instance)
                {
                    _velocity += tr.right * direction.x;
                    _velocity += tr.forward * direction.y;
                }
                else
                {
                    // Debug.LogError(
                    //     "AdvancedWalkerController : CalculateMovementDirection : TouchControls_ManipulateCharacter.Instance == NULL");
                }
            }
            else
            {
        ...
        protected virtual bool IsJumpKeyPressed()
        {
            ...
            if (characterInput is BotInput)
                return characterInput.IsJumpKeyPressed();

            if (TouchControls_ManipulateCharacter.Instance)
            {
                var isJump = TouchControls_ManipulateCharacter.Instance.GetIsJumpingPressed() ||
                             characterInput.IsJumpKeyPressed();
                return isJump;
            }
        ...
        void HandleJumping()
        {
            if (currentControllerState == ControllerState.Grounded || canJumpUp)
            {
        ...
        private ICharacterVelocity _velocityCharacter;

        public void UpdateVelocity(ICharacterVelocity velocity)
        {
            _velocityCharacter = velocity;
        }
    }
}