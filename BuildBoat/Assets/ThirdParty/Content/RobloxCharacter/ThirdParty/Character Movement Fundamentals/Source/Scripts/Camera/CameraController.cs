...
namespace CMF
{
	...
	public class CameraController : MonoBehaviour, ICameraMovement
	{
		...
		public CameraInput CameraInput => cameraInput == null ? cameraInput = GetComponent<CameraInput>() : cameraInput;
		
		private ICameraVelocity _cameraVelocity;
		...
		protected virtual void HandleCameraRotation()
		{
			...
			Vector2 input = _cameraVelocity.GenerateCameraVelocity();

			float _inputHorizontal = input.x;//cameraInput.GetHorizontalCameraInput();
			float _inputVertical = input.y;// cameraInput.GetVerticalCameraInput();
			...
			// 	{
			// 		_inputHorizontal = TouchControls_ManipulateCharacter.Instance.GetHorizontalCameraInput();
			// 		_inputVertical = TouchControls_ManipulateCharacter.Instance.GetVerticalCameraInput();
			//
			// 		Debug.Log(_inputHorizontal + "    " + _inputVertical);
			// 	}
			// 	else Debug.LogError("CameraController : HandleCameraRotation : TouchControls_ManipulateCharacter.Instance == NULL");
			// }
			
			if(!_isCameraLocked) RotateCamera(_inputHorizontal, _inputVertical);
		}
