Shader "Toony Colors Pro 2/Examples/Cat Demo/Water"
{
    Properties
    {
        ...
        // Добавляем свойства для передачи координат X и Z и получения высоты Y
        _PositionX ("Position X", Float) = 0
        _PositionZ ("Position Z", Float) = 0
        _WaterHeight ("Water Height", Float) = 0 // Добавлено в Properties
    }
    ...
    SubShader
    {
        ...
        // Добавляем переменные для передачи координат X и Z и получения высоты Y
        float _PositionX;
        float _PositionZ;
        float _WaterHeight;
        ...
        {
            // Рассчитываем высоту воды в заданной точке
            float3 worldPos = float3(_PositionX, 0, _PositionZ);
            float3 _pos = worldPos.xyz * _WaveFrequency;
            float _phase = TIME * _WaveSpeed;
            half4 vsw_offsets = half4(1.0, 2.2, 0.6, 1.3);
            half4 vsw_ph_offsets = half4(1.0, 1.3, 2.2, 0.4);
            ...
            float waveFactorX = dot(waveXZ.xy, 1) * _WaveHeight / 2;
            float waveFactorZ = dot(waveXZ.zw, 1) * _WaveHeight / 2;
            _WaterHeight = (waveFactorX + waveFactorZ) * 1.0; // VSW_STRENGTH is assumed to be 1
